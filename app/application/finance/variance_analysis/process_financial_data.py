# app/application/finance/use_cases/process_financial_data.py
...
from app.infrastructure.persistence.excel.data_utils import DEFAULT_CONFIG, EXCEL_PROCESSING, FILE_PROCESSING
from app.infrastructure.persistence.excel.excel_processing import (
...
from app.domain.finance.variance_analysis.services.anomaly_detection import build_anoms_python_mode, build_anoms_ai_mode
from app.domain.finance.variance_analysis.services.revenue_analyzer import analyze_comprehensive_revenue_impact_from_bytes, analyze_revenue_variance_comprehensive
from app.infrastructure.persistence.excel.revenue_variance_excel import _add_revenue_variance_analysis_to_sheet
...
def process_all_python_mode(
    ...
    for fname, xl_bytes in files:
        ...
        if bs_sheet:
            try:
                ...
                logger.warning(f"Failed to load BS sheet '{bs_sheet}': {str(e)}")
        else:
            logger.warning(f"No Balance Sheet found in {fname}")
        ...
        if pl_sheet:
            try:
                ...
                logger.warning(f"Failed to load PL sheet '{pl_sheet}': {str(e)}")
        else:
            logger.warning(f"No Profit & Loss sheet found in {fname}")
        ...
        for idx, (fname, xl_bytes) in enumerate(files):
            ...
            if bs_sheet:
                try:
                    ...
                    logger.warning(f"Could not add cleaned BS sheet for {sub}: {e}")
            ...
            if pl_sheet:
                try:
                    ...
                    logger.warning(f"Could not add cleaned PL sheet for {sub}: {e}")

    except Exception as e:
        logger.error(f"Error adding cleaned sheets: {e}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"Months Analysis sheets creation failed: {e}", exc_info=True)
...
def process_all_ai_mode(
    ...
    for file_idx, (fname, xl_bytes) in enumerate(files, 1):
        ...
        from app.infrastructure.external.ai_analyzer import LLMFinancialAnalyzer
        ...
        try:
            ...
            logger.warning(f"Could not collect BS/PL data for {sub}: {e}")
    ...
    from app.domain.finance.variance_analysis.services.variance_detector import create_excel_output
